...
namespace SalesManagement_SysDev
{
    public partial class F_AdSalesOffice : Form
    {
        ...
        public F_AdSalesOffice(F_AdMenu adMenu, string ID, string Name)
        {
            InitializeComponent();
            AdMenu = adMenu;
            Text = "営業所管理";
        ...
        private void SetDataGridView()
        {
            ...
            {
                case "降順":
                    dataGridViewDsp.DataSource = SalesOffice.AsEnumerable().Reverse().Skip(pageSize * pageNo).Take(pageSize).ToList();
                    break;
                case "昇順":
            ...
            dataGridViewDsp.Columns[1].Width = 250;
            dataGridViewDsp.Columns[2].Width = 400;
            dataGridViewDsp.Columns[3].Width = 180;
            dataGridViewDsp.Columns[4].Width = 180;
            dataGridViewDsp.Columns[5].Width = 200;
            dataGridViewDsp.Columns[6].Width = 170;
            dataGridViewDsp.Columns[7].Width = 357;
       ...
       　}
        ...
        private void txbKeyID_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                salesOfficeDataAccess.GetSalesOfficeFlagData(sender, txbFlag);
                //フラグの数値を取得
        ...
        {
            commonModule.FirstPageClick(txbPageSize, txbPageNo, dataGridViewDsp, new List<object>(SalesOffice), btnSort);
        }

        private void btnPreviousPage_Click(object sender, EventArgs e)
        {
            commonModule.PreviousPageClick(txbPageSize, txbPageNo, dataGridViewDsp, new List<object>(SalesOffice), btnSort);
        }

        private void btnNextPage_Click(object sender, EventArgs e)
        {
            commonModule.NextPageClick(txbPageSize, txbPageNo, dataGridViewDsp, new List<object>(SalesOffice), btnSort);
        }

        private void btnLastPage_Click(object sender, EventArgs e)
        {
            commonModule.LastPageClick(txbPageSize, txbPageNo, dataGridViewDsp, new List<object>(SalesOffice), btnSort);
        }
        ...
        private void GenerateDataAdDisplay()
        {
            ...
            M_SalesOffice selectCondition = new M_SalesOffice()
            {
                SoID = 0,
                SoName = String.Empty,
                SoAddress = String.Empty,
                SoPhone = String.Empty,
                SoPostal = String.Empty,
                SoFAX = String.Empty,
                SoFlag = Flag,
                SoHidden = null
            };
        ...
        private void SetSelectData()
        {
            ...
            {
                case "降順":
                    dataGridViewDsp.DataSource = SalesOffice.AsEnumerable().Reverse().Take(pageSize).ToList();
                    break;
                case "昇順":
        ...
        {
            var regSo = GenereteDataAdRegistration();
            RegistrationSalesOffice(regSo);
        }
        ...
        private M_SalesOffice GenereteDataAdRegistration()
        {
            return new M_SalesOffice
            {
                ...
                SoPostal = txbPostal.Text.Trim(),
                SoFAX = txbFAX.Text.Trim(),
                SoFlag = 0,
                SoHidden = null
            };
        }

        private void RegistrationSalesOffice(M_SalesOffice regSo)
        {
            DialogResult result = messageDsp.MsgDspQ("M0001");
            if (result != DialogResult.OK)
                return;

            bool flg = salesOfficeDataAccess.AddSalesOfficeData(regSo);
            if (flg)
            {
        ...
        private void UpdateSalesOffice(M_SalesOffice updSo)
        {
            ...
            DialogResult result = messageDsp.MsgDspQ(MsgCode + "001", lblSoID, txbSoID);
            //●●ID:00の情報を更新しますか？
            //●●ID:00を非表示にしますか？
            ...
            if (flg)
            {
                ...
                operationHistoryDataAccess.AddOperationHistoryData(regOh);
                    ...
                    result = messageDsp.MsgDsp(MsgCode + "002", lblSoID, txbSoID);
                //●●ID:00の情報を更新しました。
                //●●ID:00を非表示にしました。
            ...
            {
                    result = messageDsp.MsgDsp(MsgCode + "003", lblSoID, txbSoID);
                //●●ID:00の情報を更新に失敗しました。
                //●●ID:00を非表示に失敗しました。
                return;
            }
